'use client';

import {
	ViewField,
	ViewSection,
} from '@/app/(dashboard)/_components/view-detail';
import { formatDate } from '@/helpers/date.helper';
import { DisplayStatus } from '@/helpers/display.helper';
import type { WarehouseModel } from '@/models/warehouse.model';

export function ViewWarehouse({ entry }: { entry: WarehouseModel }) {
	return (
		<div className="space-y-6">
			<div className="flex items-center gap-2 border-b border-line pb-4">
				<span className="font-semibold">ID</span> {entry.id}
				<div className="max-w-60 ml-2">
					<DisplayStatus
						status={entry.status}
						dataSource="warehouse"
					/>
				</div>
			</div>

			<ViewSection title="Info">
				<ViewField label="Code" value={entry.code} />
				<ViewField label="Name" value={entry.name} />
				<ViewField
					label="Default"
					value={entry.is_default ? 'Yes' : 'No'}
				/>
			</ViewSection>

			<ViewSection title="Address">
				{/*
				 * The joined address is the narrow select the backend sends with a warehouse -
				 * it carries no city row, so the street line is the whole label available here.
				 */}
				<ViewField
					label="Details"
					value={entry.address?.details ?? '-'}
				/>
				<ViewField
					label="Postal Code"
					value={entry.address?.postal_code ?? '-'}
				/>
			</ViewSection>

			{entry.notes && (
				<ViewSection title="Notes">
					<ViewField label="Notes" value={entry.notes} />
				</ViewSection>
			)}

			<ViewSection title="Timestamps">
				<ViewField
					label="Created At"
					value={formatDate(entry.created_at, 'date-time')}
				/>
				<ViewField
					label="Updated At"
					value={formatDate(entry.updated_at, 'date-time')}
				/>
				{entry.deleted_at && (
					<ViewField
						label="Deleted At"
						value={
							<span className="text-danger">
								{formatDate(entry.deleted_at, 'date-time')}
							</span>
						}
					/>
				)}
			</ViewSection>
		</div>
	);
}
