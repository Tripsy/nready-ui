...
export const Icons = {
	...
	MoreVertical: createIcon(EllipsisVertical),
	...
	CopyLink: createIcon(Link2),
	ExternalLink: createIcon(ExternalLink),
	Print: createIcon(Printer),
	Image: createIcon(Camera),
	Publishing: createIcon(Rss),
	Featured: createIcon(Star),
	// A star rating renders three states out of two icons: an outline, a half, and the outline
	// again with `fill-current` - lucide ships no solid star.
	Star: createIcon(Star),
	StarHalf: createIcon(StarHalf),
	Comment: createIcon(MessageCircleMore),
	Complaint: createIcon(TriangleAlert),
	Rating: createIcon(SmilePlus),
	Review: createIcon(MessageSquareQuote),
	...
	Action: {
		...
		Add: createIcon(Plus),
		// The other half of a quantity stepper; `Minimize` is the same glyph but means a window
		Subtract: createIcon(Minus),
		...
		SubmitReview: createIcon(Send),
		RequestRevision: createIcon(Undo2),
		MarkReady: createIcon(CircleCheck),
		Close: createIcon(Lock),
		Setup: createIcon(FileCog),
		Return: createIcon(Undo2),
		Attributes: createIcon(ListChecks),
		...
		Resolve: createIcon(CircleCheck),
